  ...
  <div class="rfm-page">
    ...
    <el-card shadow="never" style="margin-top:16px">
      ...
      <el-table :data="list" stripe size="small" max-height="450">
        ...
        <el-table-column label="R" width="60"><template #default="{row}"><el-tag :type="row.rScore>=4?'success':row.rScore>=3?'':'danger'" size="small">{{ row.rScore }}</el-tag></template></el-table-column>
        <el-table-column label="F" width="60"><template #default="{row}"><el-tag :type="row.fScore>=4?'success':row.fScore>=3?'':'danger'" size="small">{{ row.fScore }}</el-tag></template></el-table-column>
        <el-table-column label="M" width="60"><template #default="{row}"><el-tag :type="row.mScore>=4?'success':row.mScore>=3?'':'danger'" size="small">{{ row.mScore }}</el-tag></template></el-table-column>
        <el-table-column label="分层" width="130"><template #default="{row}"><el-tag :color="tc[row.segment]||'#909399'" effect="dark" size="small">{{ row.segment }}</el-tag></template></el-table-column>
        <el-table-column label="等级" width="70"><template #default="{row}">{{ row.level }}</template></el-table-column>
        <el-table-column label="来源" width="70"><template #default="{row}">{{ row.source }}</template></el-table-column>
      </el-table>
    </el-card>
  </div>
</template>
...
const tierStats = computed(() => {
  const m = {}
  list.value.forEach(d => { m[d.segment] = (m[d.segment]||0)+1 })
...
function draw() {
  if (!sRef.value || !pRef.value || !list.value.length) return
  ...
  sc.setOption({
    ...
    xAxis:{name:'R值',max:6,min:0},yAxis:{name:'F值',max:6,min:0},
    series:Object.entries(tc).map(([t,c])=>({
      name:t,type:'scatter',symbolSize:d=>(d[2]||1)*8+8,itemStyle:{color:c},
...
onMounted(async () => {
  ...
  try {
    ...
    if (Array.isArray(res) && res.length > 0) {
      list.value = res; await nextTick(); draw(); return
    }
  } catch {}
  // Fallback demo
  list.value = D; await nextTick(); draw()
})
