      ...
      <div class="panel">
        ...
        <div class="activity-list">
          <div v-for="(a,i) in activities" :key="i" class="activity">
            ...
            <div class="activity-amount" v-if="a.amount">+¥{{ a.amount.toLocaleString() }}</div>
          </div>
        </div>
      </div>
    </div>

    <!-- 第四行：今日优先级 (左) + 热销商品 (中) + 待办 (右) -->
    <div class="row-3">
      <div class="panel">
        <div class="panel-hd">
          <h4>今日优先级</h4>
        ...
        <div class="top-list">
          <div v-for="(p,i) in topProducts" :key="i" class="top-item">
            ...
            <div class="top-info">
              ...
              <div class="top-meta">销量 {{ p.sales }} · 库存 {{ p.stock }}</div>
            </div>
            <div class="top-rev">¥{{ (p.revenue/1000).toFixed(1) }}k</div>
          </div>
        </div>
      </div>
      ...
      <div class="panel">
        ...
        <div class="team-list">
          <div v-for="(t,i) in teamRank" :key="i" class="team-item">
...
const sources = [
  { name:'官网咨询', value:38, color:'#3b82f6' },
  { name:'微信营销', value:24, color:'#22c55e' },
  { name:'转介绍',   value:18, color:'#f59e0b' },
  { name:'电话来访', value:12, color:'#8b5cf6' },
  { name:'其他',     value:8,  color:'#94a3b8' },
]

const activities = [
  { who:'张磊',   action:'签下订单', target:'华润集团', amount:186000, time:'5 分钟前', color:'#3b82f6' },
  { who:'李娜',   action:'跟进客户', target:'比亚迪供应链', time:'12 分钟前', color:'#ec4899' },
  { who:'王伟',   action:'创建合同', target:'腾讯云', amount:96000, time:'38 分钟前', color:'#8b5cf6' },
  { who:'陈晓',   action:'备注更新', target:'字节跳动', time:'1 小时前', color:'#22c55e' },
  { who:'赵敏',   action:'签下订单', target:'招商银行', amount:245000, time:'2 小时前', color:'#f59e0b' },
  { who:'孙强',   action:'新建客户', target:'宁德时代', time:'3 小时前', color:'#06b6d4' },
]

const topProducts = [
  { name:'YONEX 羽毛球拍 弓箭系列', sales:248, stock:32,  revenue:148800 },
  { name:'Duracell 5号电池 24节装', sales:189, stock:520, revenue:56700 },
  { name:'李宁 运动护腕 双只装',   sales:156, stock:88,  revenue:12480 },
  { name:'尤尼克斯 训练用球 6只',   sales:142, stock:64,  revenue:8520  },
  { name:'速比涛 泳镜 防雾款',       sales:128, stock:45,  revenue:16640 },
]

const teamRank = [
  { name:'张磊', deals:18, amount:286000, color:'#3b82f6' },
  { name:'李娜', deals:15, amount:234000, color:'#ec4899' },
  { name:'王伟', deals:12, amount:198000, color:'#8b5cf6' },
  { name:'陈晓', deals:9,  amount:142000, color:'#22c55e' },
  { name:'赵敏', deals:7,  amount:98000,  color:'#f59e0b' },
]
...
async function load() {
  ...
  try {
    const p = await aiApi.todayPriority()
    priorityList.value = (p || []).slice(0, 5)
  } catch {}
...
function drawDonut() {
  ...
  donutChart.setOption({
    ...
    series:[{type:'pie',radius:['58%','78%'],center:['50%','50%'],avoidLabelOverlap:false,
      ...
      data:sources.map(s=>({value:s.value,name:s.name,itemStyle:{color:s.color}}))}]
  })
}
...
.top-item{display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:6px;transition:background .15s}
